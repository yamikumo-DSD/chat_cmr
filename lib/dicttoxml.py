...
class IgnoredKey:
    ...
    def __str__(self) -> str:
        return f"IgnoredKey(value={self._value})"
...
def convert_dict(obj, ids, parent, attr_type, item_func, cdata):
    ...
    for key, val in obj.items():
        ...
        if type(val) == bool:
            ...
            addline(convert_bytes(key, val, attr_type, cdata, attr))
        ...
        elif isinstance(val, IgnoredKey):
            addline(convert_ignored_key(key, val, attr_type, cdata, attr))
            
        elif val is None:
            addline(convert_none(key, val, attr_type, cdata, attr))

        else:
            addline(convert_unsupported(key, val, attr_type, cdata, attr))
...
def convert_list(items, ids, parent, attr_type, item_func, cdata):
    ...
    for i, item in enumerate(items):
        ...
        if isinstance(item, numbers.Number) or type(item) in (str, unicode):
            ...
            addline(convert_bytes(item_name, item, attr_type, cdata, attr))
        ...
        elif isinstance(item, IgnoredKey):
            addline(convert_ignored_key(item_name, item, attr_type, cdata, attr))

        elif item is None:
            addline(convert_none(item_name, None, attr_type, cdata, attr))

        else:
            addline(convert_unsupported(item_name, item, attr_type, cdata, attr))
    return ''.join(output)


def convert_bytes(key, val, attr_type, cdata=False, attr=None):
    """Converts a bytes into an XML element"""
    LOG.info('Inside convert_bytes(): key="%s", val="%s", type(val) is: "%s"' % (
        unicode_me(key),
        unicode_me(val),
        type(val).__name__
        )
...
def convert_ignored_key(key, val, attr_type, cdata=False, attr=None):
    return ''


def convert_unsupported(key, val, attr_type, cdata=False, attr=None):
    """Converts a unsupported type into an XML element"""
    LOG.info('Inside convert_unsupported(): key="%s", val="%s", type(val) is: "%s"' % (
        unicode_me(key),
        unicode_me(val),
        type(val).__name__
        )
...
def convert_none(key, val, attr_type, cdata=False, attr=None):
    ...
    if attr_type:
        attr['type'] = get_xml_type(val)
    attrstring = make_attrstring(attr)
    return '<%s%s>None</%s>' % (key, attrstring, key)
