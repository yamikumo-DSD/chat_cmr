...
def stringify_html_elements(elems):
    def stringify_html_element(elem):
        if isinstance(elem, str):
            return elem
        elif elem["tagName"] == "br":
            return "\n"
        else:
            content = "".join(
                stringify_html_element(child) for child in elem["content"]
            )
            ...
            if elem["tagName"] == "a":
                ...
                return format_str.format(content, href or "")
            else:
...
def extract_html_elements(html: str) -> list[dict]:
    from bs4 import BeautifulSoup
    
    soup = BeautifulSoup(html, "html.parser")
    removed_tags = ["script", "style", "button", "header", "footer", "nav", "iframe"]
    for script_or_style in soup(removed_tags):
        script_or_style.decompose()

    # Find all text elements and extract their tag name, attributes, and content
    text_elements = soup.find_all(text=True)
    result = []
    for elem in text_elements:
        tag_name = str(elem.parent.name)
        attrs = {attr: str(elem.parent[attr]) for attr in elem.parent.attrs}
        content = elem
        entry = {"tagName": tag_name, "attributes": attrs, "content": content}
        result.append(entry)
