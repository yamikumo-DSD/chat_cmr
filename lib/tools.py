...
def summarize(
    ...
    SUMMARIZE_PROMPT_TEMPLATE_WITH_QUERY = '''次の文章を、リクエストを主体に日本語で要約を作成してください。
内容はできるだけ具体的に記述してください。
-------------------
{text}
-------------------
...
要約:'''
    ...
    SUMMARIZE_PROMPT_TEMPLATE_WITHOUT_QUERY = '''次の文章の要約を日本語で作成してください。
内容はできるだけ具体的に記述してください。
-------------------
{text}
-------------------
...
要約:'''
    ...
    if query:
        prompt = SUMMARIZE_PROMPT_TEMPLATE_WITH_QUERY.format(text=text, query=query)
    else:
        prompt = SUMMARIZE_PROMPT_TEMPLATE_WITHOUT_QUERY.format(text=text)
